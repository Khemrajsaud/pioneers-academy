  ...
  return (
    <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
      {[...Array(count)].map((_, i) => (
        <div
          key={i}
          className="absolute w-1 h-1 bg-linear-to-r from-pink-400 to-purple-400 rounded-full animate-float opacity-30"
          style={{
            left: `${Math.random() * 100}%`,
            top: `${Math.random() * 100}%`,
...
export const GradientOverlay = ({ position = 'top', opacity = 0.1, className = '' }) => {
  const positions = {
    top: 'bg-linear-to-b from-pink-500/10 via-purple-500/5 to-transparent',
    bottom: 'bg-linear-to-t from-pink-500/10 via-purple-500/5 to-transparent',
    left: 'bg-linear-to-r from-pink-500/10 via-purple-500/5 to-transparent',
    right: 'bg-linear-to-l from-pink-500/10 via-purple-500/5 to-transparent',
