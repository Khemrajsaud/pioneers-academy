...
const Navbar = () => {
  ...
  return (
    <>
      {/* Admission Bar - scrolls out of view naturally (NOT sticky) */}
      <div className="overflow-hidden border-b border-[color:var(--border)] bg-gradient-to-r from-[var(--blue3)] to-[color:var(--primary)] text-white">
        <div className="admission-marquee">
          {[0, 1].map((item) => (
            <div key={item} className="flex shrink-0 items-center gap-2 px-8 py-2 text-xs font-semibold sm:text-sm">
              <Megaphone size={16} className="shrink-0 animate-pulse-subtle" />
              <span>
                {language === "en" ? "🎓 School Admission Open for 2026 - Apply Now!" : "विद्यालय भर्ना २०८३ का लागि खुला छ"}
              </span>
            </div>
          ))}
        </div>
      ...
      {/* Main Navbar - sticky, stays at top when admission bar scrolls away */}
      <nav ref={navRef} className="sticky top-0 z-50 border-b border-[color:var(--border)] bg-[color:var(--bg)]/95 backdrop-blur-md shadow-sm">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="hidden md:flex items-center gap-2">
              ...
              <button
                ...
                className="inline-flex items-center justify-center gap-1.5 rounded-md border border-[color:var(--border)] bg-[color:var(--card)] px-3 py-1.5 text-[color:var(--text)] hover:bg-[color:var(--primary)] hover:text-white hover:border-[color:var(--primary)] font-semibold text-sm transition-all duration-200"
                aria-label="Toggle language"
              >
                <img
                  src={language === "ne" ? Nepal : English}
                  alt={language === "ne" ? "Nepal" : "English"}
          ...
          <div className="md:hidden absolute left-0 top-full w-full border-b border-[color:var(--border)] bg-[color:var(--bg)] text-[color:var(--text)] flex flex-col space-y-1 px-4 py-3 shadow-lg animate-slideInDown">
            ...
            <div className="mt-2 pt-2 border-t border-[color:var(--border)]">
              <button
                type="button"
                onClick={toggleLanguage}
                className="w-full inline-flex items-center justify-center gap-2 rounded-md border border-[color:var(--border)] bg-[color:var(--card)] px-4 py-2.5 text-[color:var(--text)] hover:bg-[color:var(--primary)] hover:text-white hover:border-[color:var(--primary)] font-semibold text-sm transition-all duration-200"
                aria-label="Toggle language"
              >
                <img
                  src={language === "ne" ? Nepal : English}
                ...
                />
                <span>{language === "ne" ? "Switch to English" : "Switch to Nepali (नेपाली)"}</span>
              </button>
            </div>
          </div>
        )}
